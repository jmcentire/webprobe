...
class _MockCtxManager:
    """Helper: async context manager wrapping a mock response."""
...
def _make_mock_session(response):
    ...
    session = MagicMock()
    session.get = MagicMock(return_value=_MockCtxManager(response))
    return session
...
class TestCrawlPass:
    ...
    async def test_crawl_pass_basic(self):
        ...
        with patch('src.webprobe.mapper._fetch') as mock_fetch:
            # Mock responses
...
def _make_node(url, requires_auth=None, discovered_via=DiscoveryMethod.crawl):
    """Create a real Node instance for test assertions."""
    return Node(
        id=url,
...
def _mock_auth_config(method='none', **kwargs):
    """Create a mock auth config with sensible defaults."""
    cfg = Mock()
    cfg.method = method
    cfg.cookie_name = kwargs.get('cookie_name', '')
    cfg.cookie_value = kwargs.get('cookie_value', '')
    cfg.bearer_token = kwargs.get('bearer_token', '')
    cfg.header_name = kwargs.get('header_name', '')
    cfg.header_value = kwargs.get('header_value', '')
    cfg.login_url = kwargs.get('login_url', '/login')
    cfg.auth_indicator = kwargs.get('auth_indicator', '')
    cfg.credentials = kwargs.get('credentials', [])
    return cfg
...
class TestMapSite:
    ...
    async def test_map_site_full_flow(self):
        ...
        with patch('src.webprobe.mapper._fetch', new_callable=AsyncMock) as mock_fetch, \
             patch('src.webprobe.mapper._crawl_pass', new_callable=AsyncMock) as mock_crawl:

            async def fetch_side_effect(session, url, delay=0):
            ...
            node = _make_node('http://example.com/', requires_auth=False)
            mock_crawl.return_value = ({'http://example.com/': node}, [])
            ...
            mock_config.crawl.follow_external = False
            mock_config.crawl.url_exclude_patterns = []
            mock_config.auth = _mock_auth_config('none')
    ...
    async def test_map_site_authenticated_crawl(self):
        ...
        with patch('src.webprobe.mapper._fetch', new_callable=AsyncMock) as mock_fetch, \
            ...
            mock_config.crawl.follow_external = False
            mock_config.crawl.url_exclude_patterns = []
            mock_config.auth = _mock_auth_config('bearer', bearer_token='test-token')
    ...
    async def test_map_site_merge_auth_nodes(self):
        ...
        with patch('src.webprobe.mapper._fetch', new_callable=AsyncMock) as mock_fetch, \
            ...
            public_node = _make_node('http://example.com/public', requires_auth=False)

            # Private node only in authenticated pass
            private_node = _make_node('http://example.com/private', requires_auth=True)
...
class TestInvariants:
    ...
    async def test_invariant_all_urls_normalized(self):
        ...
        with patch('src.webprobe.mapper._fetch', new_callable=AsyncMock) as mock_fetch, \
            ...
            node = _make_node('http://example.com/path', requires_auth=False)
            mock_crawl.return_value = ({'http://example.com/path': node}, [])
